...
if (text.includes(prefix + "opromote")) {
...
if (text.includes(prefix + "odemote")) {
	...
	if (!isGroup) return conn.sendMessage(id, "Ini Bukan Grup Bodo!", MessageType.text , {quoted: m})
		...
		if (args[0].startsWith('08')) return conn.sendMessage(id, 'Gunakan kode negara mas', MessageType.text, {quoted: m})
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
 await conn.groupDemoteAdmin(from, [num, num])
 .catch(
          (error) => {
...
if (text.includes(prefix + "owelcome")) {
	if (!isGroup) return conn.sendMessage(id, "Ini Bukan Grup Bodo!", MessageType.text , {quoted: m})
		if (!isBotGroupAdmins) return conn.sendMessage(id, "Bot Tidak Menjadi Admin Silakan Adminkan Untuk Bisa Menggunakan Fitur Ini", MessageType.text, { quoted: m })
		if (!isOwner) return conn.sendMessage(id , "Fitur Ini Khusus Pembuat Chopper!" , MessageType.text, {quoted: m})		            
					if (args.length < 1) return conn.sendMessage(id, 'Hmmmm', MessageType.text, {quoted: m})
					if (Number(args[0]) === 1) {
						if (isWelkom) return conn.sendMessage(id,"Udah Aktif", MessageType.text, {quoted: m})
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						conn.sendMessage(id, 'Sukses mengaktifkan fitur welcome di group ini ✔️', MessageType.text, {quoted: m})
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						conn.sendMessage(id , 'Sukses menonaktifkan fitur welcome di group ini ✔️', MessageType.text, {quoted: m})
					} else {
						conn.sendMessage(id, '1 untuk mengaktifkan, 0 untuk menonaktifkan' , MessageType.text, {quoted: m})
					}
					}										

if (text.includes(prefix + "prefix")) {
if (args.length < 1) return
if (!isOwner) return conn.sendMessage(id , "Lahh!?, Lu Siapa?" , MessageType.text, {quoted: m})
prefix = args[0]
conn.sendMessage(id, `Prefix berhasil di ubah menjadi : ${prefix}`,MessageType.text, {quoted: m})
}
...
conn.sendMessage(id, info.info(id, BotName, tampilTanggal, tampilWaktu, instagramlu, whatsapplu, kapanbotaktif, grupch1, grupch2) ,MessageType.text, { quoted: m });
   ...
   if (type == 'videoMessage') {
				...
				if (captvid == prefix + 'stikergif' || captvid == prefix + 'stcgif') {
					try {
				...
				if (body.startsWith(prefix + 'tts ')) {
					...
					gtts.save(rendom, dtt, function () {
						exec(`ffmpeg -i ${rendom} -ar 48000 -vn -c:a libopus ${random}`, (error, stdout, stder) => {
							...
							conn.sendMessage(id, res, MessageType.audio, {quoted: m, ptt: true})
 ...
 *Kecocokan berdasarkan nama*
 ...
 ${d}
   ...
   {
    var items = ["cowo ganteng", "cogan", "cowo Indonesia"];
    var cewe = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + cewe;
...
if (text.includes('>infoanime')){
  var teks = text.replace(/>infoanime /, '')
    axios.get('https://st4rz.herokuapp.com/api/dewabatch?q='+teks)
    .then((res) => {
      imageToBase64(res.data.thumb)
        .then((ress) => {
